...
export async function extractBranding(
  ...
  try {
    ...
    spinner.stop();
    // Scroll to bottom so below-fold sections (e.g. tan/mint backgrounds) are rendered
    await page.evaluate(() => {
    ...
    const [
      ...
      extractColors(page, { includeWordPressPresets: options.includeWordPressPresets }),
    ...
    // Multi-page exploration: discover and extract from product/category pages (default: on)
    if (options.explore !== false) {
      spinner.start("Discovering links...");
      const exploreUrls = await discoverExploreLinks(page, finalUrl);
      spinner.stop();

      if (exploreUrls.length > 0) {
        for (let i = 0; i < exploreUrls.length; i++) {
          try {
            spinner.start(`Exploring page (${i + 1}/${exploreUrls.length})...`);
            await page.goto(exploreUrls[i], {
              waitUntil: "domcontentloaded",
              timeout: 15000 * timeoutMultiplier,
            });
            await page.waitForTimeout(2000 * timeoutMultiplier);
            const pageColors = await extractColors(page, { includeWordPressPresets: options.includeWordPressPresets });
            spinner.stop();

            // Merge palette: sum count, areaWeight, prominence by hex
            ...
            colors.palette.forEach((c) => {
              ...
              paletteByHex.set(hex, {
                ...c,
                count: c.count,
                areaWeight: c.areaWeight || 0,
                prominence: c.prominence || 0,
              });
            });
            pageColors.palette.forEach((c) => {
              const hex = c.normalized || c.color;
              const existing = paletteByHex.get(hex);
              if (existing) {
                existing.count += c.count || 0;
                existing.areaWeight += c.areaWeight || 0;
                existing.prominence = (existing.prominence || 0) + (c.prominence || 0);
              } else {
                paletteByHex.set(hex, { ...c });
              }
            });
            colors.palette = Array.from(paletteByHex.values())
              .sort((a, b) => (b.prominence || 0) - (a.prominence || 0));
          } catch (err) {
            spinner.stop();
            console.log(chalk.hex('#FFB86C')(`  ⚠ Explore failed: ${exploreUrls[i]} - ${err.message}`));
          }
        }

        // Navigate back to homepage for hover/focus extraction
        await page.goto(finalUrl, { waitUntil: "domcontentloaded", timeout: 10000 * timeoutMultiplier });
    ...
    for (const element of sampled) {
      try {
        // Skip elements inside dialogs/popups
        const inDialog = await element.evaluate(el =>
          el.closest('dialog, [role="dialog"], [role="alertdialog"], [aria-modal="true"]') !== null
        );
    ...
    if (options.darkMode) {
      ...
      const darkModeColors = await extractColors(page, { includeWordPressPresets: options.includeWordPressPresets });
    ...
    if (options.mobile) {
      ...
      const mobileColors = await extractColors(page, { includeWordPressPresets: options.includeWordPressPresets });
...
async function extractLogo(page, url) {
  return await page.evaluate((baseUrl) => {
    ...
    const isInsideDialog = (el) => el.closest('dialog, [role="dialog"], [role="alertdialog"], [aria-modal="true"]');
...
/**
 * Discover links to explore (product/category pages) for multi-page color extraction
 * @param {import('playwright').Page} page
 * @param {string} baseUrl
 * @returns {Promise<string[]>} Up to 2 URLs to explore
 */
async function discoverExploreLinks(page, baseUrl) {
  const base = new URL(baseUrl);
  const scored = await page.evaluate((origin) => {
    const links = Array.from(document.querySelectorAll('a[href]'));
    const seen = new Set();
    const scored = [];

    const excludePaths = /^\/(cart|checkout|account|login|logout|search)(\/|$)/i;
    const productPattern = /\/(products?|p|shop)\/[^/]+\/?$/i;
    const categoryPattern = /\/(collections?|categories?|dresses|pajamas|tunics|gifts)(\/|$)/i;
    ...
    for (const a of links) {
      ...
      try {
        ...
        if (excludePaths.test(path)) continue;
        ...
        let score = 1;
        if (productPattern.test(path)) score = 3;
        else if (categoryPattern.test(path)) score = 2;
  ...
  // Dedupe by path (ignore query), pick top 2
  const byPath = new Map();
  for (const { url, score } of scored) {
    const u = new URL(url);
    const pathKey = u.pathname;
    if (!byPath.has(pathKey) || byPath.get(pathKey).score < score) {
      byPath.set(pathKey, { url, score });
    }
  }
 ...
 */
async function extractColors(page, colorOptions = {}) {
  const includeWordPressPresets = colorOptions.includeWordPressPresets === true;
  const result = await page.evaluate((includeWpPresets) => {
    ...
    for (let i = 0; i < styles.length; i++) {
      const prop = styles[i];
      if (prop.startsWith("--")) {
        // Skip WordPress presets unless explicitly requested (theme.json colors)
        if (!includeWpPresets && prop.startsWith("--wp--preset")) {
          continue;
        }
    ...
    function addColorToMap(color, score, areaDelta, context, options = {}) {
      if (!color || color === "rgba(0, 0, 0, 0)" || color === "transparent") return;
      // For stylesheet-originated colors, only accept hex/rgb/hsl (reject var(), initial, etc.)
      ...
      existing.areaWeight += areaDelta || 0;
      if (score > 1) {
    ...
    // Scan stylesheets for background-color and background (solid color) rules
    // Captures colors defined in CSS but used on few elements
    try {
      ...
      for (const sheet of sheets) {
        try {
          ...
          for (const rule of rules) {
            ...
            const bgShorthand = rule.style.background;
            // Skip gradient backgrounds - only want solid colors
            const colorValues = [
              ...extractColorsFromValue(bgColor),
              ...(bgShorthand && !bgShorthand.includes("gradient")
                ? extractColorsFromValue(bgShorthand)
                : []),
            ];
            for (const color of colorValues) {
              if (color && color !== "rgba(0, 0, 0, 0)" && color !== "transparent") {
                addColorToMap(color, 2, 0, "stylesheet", { strictFormat: true });
              }
            }
          }
        } catch (e) {
    ...
    // Viewport area for area-weighted threshold (large section backgrounds)
    ...
    elements.forEach((el) => {
      ...
      // Area for background colors: large section divs get weighted by visible area
      const rect = el.getBoundingClientRect();
      const elArea = (rect.width > 0 && rect.height > 0) ? rect.width * rect.height : 0;
      const isSectionLike = elArea > viewportArea * 0.02 &&
        (computed.display === "block" || computed.display === "flex" || computed.display === "grid");
      ...
      // Section-like elements: boost score so large background divs pass threshold
      if (isSectionLike && bgColor && bgColor !== 'rgba(0, 0, 0, 0)' && bgColor !== 'transparent') {
        score = Math.max(score, 3);
      }

      // Background colors: add with area weight (large divs contribute more)
      const bgArea = (bgColor && bgColor !== 'rgba(0, 0, 0, 0)' && bgColor !== 'transparent') ? elArea : 0;
      extractColorsFromValue(bgColor).forEach((color) => {
        addColorToMap(color, score, bgArea, context);
      });

      // Text and border colors: count only (no area weighting)
      extractColorsFromValue(textColor).forEach((color) => {
        addColorToMap(color, score, 0, context);
      });
      extractColorsFromValue(borderColor).forEach((color) => {
        addColorToMap(color, score, 0, context);
      });
    ...
    // Area threshold: colors covering >= 1% of viewport are prominent section backgrounds
    const areaThreshold = viewportArea * 0.01;
    ...
    const palette = Array.from(colorMap.entries())
      .filter(([normalizedColor, data]) => {
        // Pass if count meets threshold OR area weight meets threshold (large section backgrounds)
        const passesCount = data.count >= threshold;
        const passesArea = (data.areaWeight || 0) >= areaThreshold;
        if (!passesCount && !passesArea) return false;
      ...
      .map(([normalizedColor, data]) => {
        const areaWeight = data.areaWeight || 0;
        const areaPct = areaWeight / viewportArea * 100;
        const countBoost = Math.min(data.count / 30, 2); // Cap count contribution so area dominates
        const semanticBoost = data.score > 20 ? 10 : data.score > 5 ? 5 : 0;
        // Penalize colors used on many small elements (icons, labels) — high count, tiny area
        const smallAccentPenalty = (data.count > 15 && areaWeight < viewportArea * 0.005) ? 4 : 0;
        let prominence = areaPct * 2 + countBoost + semanticBoost - smallAccentPenalty;

        // Deprioritize pure white/black — generic scaffolding, not brand identity
        const PURE_NEUTRALS = new Set(['#ffffff', '#fff', '#000000', '#000']);
        const toSixDigitHex = (hex) => {
          if (hex.length === 4) return '#' + hex[1].repeat(2) + hex[2].repeat(2) + hex[3].repeat(2);
          return hex;
        };
        const hex6 = toSixDigitHex(normalizedColor);
        if (PURE_NEUTRALS.has(hex6)) {
          const isBrandBlack = hex6 === '#000000' && areaWeight > viewportArea * 0.05;
          const hasStrongSemantic = hex6 === '#000000' && data.score > 20 && areaWeight > 0; // Black on large areas with semantic context
          if (!isBrandBlack && !hasStrongSemantic) {
            prominence = 0; // Demote so they fall to secondary/accent (stays low when merged across pages)
          }
        }

        // Infer usage for clearer labels
        let usage = "accent";
        if (areaWeight > viewportArea * 0.02) usage = "background";
        else if (areaWeight > 0) usage = "background";
        else if (data.count > 20) usage = "text, icons";
        return {
          color: data.original,
    ...
    palette.forEach((color, index) => {
      ...
      // Keep the one with highest prominence (most visually important)
      const best = similar.sort((a, b) => (b.prominence || 0) - (a.prominence || 0))[0];
...
/**
 * Detect if site uses light or dark theme based on body background and text
 */
async function extractTheme(page) {
  return await page.evaluate(() => {
    ...
    const toRgb = (c) => {
      ...
      const hex = c.match(/#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})/);
    ...
    const luminance = (rgb) => {
      if (!rgb) return 0.5;
      const [r, g, b] = [rgb.r, rgb.g, rgb.b].map((x) => x / 255);
      return 0.299 * r + 0.587 * g + 0.114 * b;
    };

    const bgRgb = toRgb(bg);
    const textRgb = toRgb(textColor);
    const bgLum = bgRgb ? luminance(bgRgb) : 0.98;
    const textLum = textRgb ? luminance(textRgb) : 0.1;

    if (bg === 'transparent' || bg === 'rgba(0, 0, 0, 0)') {
      const htmlBg = getComputedStyle(html).backgroundColor;
      const htmlRgb = toRgb(htmlBg);
      const htmlLum = htmlRgb ? luminance(htmlRgb) : 0.98;
      return { theme: htmlLum < 0.5 ? 'dark' : 'light' };
    }

    return { theme: bgLum < 0.5 ? 'dark' : 'light' };
  });
}
...
async function extractShadows(page) {
  return await page.evaluate(() => {
    ...
    const getContext = (el) => {
      ...
      const classes = Array.from(el.classList);
      if (role) return role;
      if (classes.some(c => c.includes('button') || c.includes('btn'))) return 'button';
      if (classes.some(c => c.includes('card'))) return 'card';
      if (classes.some(c => c.includes('modal') || c.includes('dialog'))) return 'modal';
      if (classes.some(c => c.includes('dropdown') || c.includes('menu'))) return 'dropdown';
      return tag;
    };
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      if (shadow && shadow !== "none") {
        if (!shadows.has(shadow)) {
          shadows.set(shadow, { count: 0, elements: new Set() });
        }
        const data = shadows.get(shadow);
        data.count++;
        data.elements.add(getContext(el));
      }
    });
...
async function extractBadgeStyles(page) {
  return await page.evaluate(() => {
    ...
    badges.forEach((badge) => {
      if (isInsideDialog(badge)) return;
